...
class Spotify {
	...
	constructor(id){
		...
		this.spotify = document.createElement('div')
		this.spotify.className = "module spotify"
		this.is_playing = false
		this.load = [0,6]
		this.scene.append(this.spotify)
		this.initial()
	}
	...
	startTimer() {
		try {
			this.stopTimer()
			this.getTrack()
			this.timer = setInterval(this.getTrack.bind(this), this.is_playing ? 3000 : 10000)
		} catch (e){
			alert(e)
		}
	}
	...
	timerSmoothFunc() {
		this.progress += 100
		this.group.progress.innerHTML = this.ms2s(this.progress)
		this.group.duration_progress.style.width = this.progress/this.duration * 100 + '%'
	}
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (this.is_playing) {
						...
						if (this.timerSmooth == null) {
							this.timerSmooth = setInterval(this.timerSmoothFunc.bind(this), 100)
						}
					} else {

						clearInterval(this.timerSmooth)
						this.timerSmooth = null
